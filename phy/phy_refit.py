...
class PhyRefitG2VScene(Scene):
    def construct(self):
        ...
        txt10 = VGroup(Text("当电流一定时，", t2c = { "电流一定": RED }), Tex("U", color = YELLOW_B), Text("越大，"), Tex("R", color = GREEN), Text("越大")).arrange().scale(0.8)
        ...
        self.wait(0.6)
        self.play(
            vgTxt7_8.animate.scale(0.8).next_to(txtG2V, DOWN, LARGE_BUFF * 1.7, LEFT),
            Group(gPhyG, rgU, texUg, texUc).animate.shift(UP * 1.2 + RIGHT * 1.5), run_time = 0.8)
        ...
        texRbL.next_to(texUcL, UP)
        texCompare2.move_to((texRgL.get_center() + texRbL.get_center()) / 2)
        self.play(FadeIn(texRgL, DOWN), FadeIn(texRbL, DOWN), FadeIn(texCompare2))
        self.play(ShowCreationThenFadeAround(txt10[-1:-3:-1]), ShowCreationThenFadeAround(txt7[-2:-9:-1]))
        self.wait()
        self.play(*[FadeOut(m) for m in [txt7, txt8, txt9, txt10, lineUg, lineUc, lineU, texUgL, texUcL, texUL, texRgL, texRbL, texCompare1, texCompare2]])
